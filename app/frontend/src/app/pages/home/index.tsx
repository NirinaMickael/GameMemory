...
import { gameSettingSchema, type gameSettingValues } from "@/features/setting/data/gameSetting.schema";
...
export  function HomePage() {
  ...
  useEffect(() => {
    if (savedGame) {
      const hoursSinceLastSave = (Date.now() - savedGame.savedAt) / (1000 * 60 * 60);
  ...
  const onSubmit = (data: gameSettingValues) => {
    const players: Player[] = data.playerNames.slice(0, playerCount).map((name, index) => ({
      id: index,
      name,
      score: 0,
    ...
    <div className="min-h-[calc(100vh-3.5rem)] bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-purple-900 dark:to-indigo-900 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl space-y-8">
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300">{t('game.gameSubtitle')}</p>
        </div>

        {savedGame && (
          <SavedGameCard savedGame={savedGame} onResume={handleResumeSavedGame} onDiscard={clearSavedGame} />
        )}
            ...
            <FormProvider {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <div className="space-y-3">
                  <Label className="text-base font-semibold">{t('game.cardTheme')}</Label>
                  <RadioGroup
                    value={form.watch("theme")}
                    onValueChange={(val) => form.setValue("theme", val as gameSettingValues["theme"])}
                    className="grid grid-cols-2 gap-4"
                  >
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="numbers" id="numbers" />
                      <Label htmlFor="numbers" className="cursor-pointer flex items-center space-x-2">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Label htmlFor="icons" className="cursor-pointer flex items-center space-x-2">
                        <span className="text-2xl">🎨</span>
                ...
                {playerCount > 1 && (
                  <div className="space-y-3">
                    <Label className="text-base font-semibold">{t('game.playerNames')}</Label>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {Array.from({ length: playerCount }, (_, i) => (
                        <div key={i} className="flex items-center space-x-2">
                          <div className={`w-4 h-4 rounded-full ${PLAYER_COLORS[i]}`} />
                          <ControlledInput
                            name={`playerNames.${i}`}
                            control={form.control}
                            placeholder={formatMessage("Joueur {number}", { number: i + 1 })}
                            className="flex-1"
                          />
                        </div>
                      ))}
